...
def main(args=None):
    ...
    dataloader_train = DataLoader(dataset_train, num_workers=3, collate_fn=collater_train, batch_sampler=sampler)
    ...
    for epoch_num in range(parser.epochs):
        ...
        for iter_num, data in enumerate(dataloader_train):
            ...
            try:
                ...
                fps = (the_length - iter_num - 1) / (time.time() - now_time) / 60
                m, s = divmod(fps, 60)
                h, m = divmod(m, 60)
                ...
                print(
                    'Epoch: {} | Iteration: {} | Classification loss: {:1.5f} | Regression loss: {:1.5f} | Running loss: {:1.5f}'.format(
                        epoch_num, iter_num, float(classification_loss), float(regression_loss), np.mean(loss_hist))," | Rest time : ","%02d:%02d:%02d" % (h, m, s))
        ...
        if parser.dataset == 'coco':
            ...
            a = coco_eval.evaluate_coco(dataset_val, retinanet)
            with open(parser.name + str(epoch_num) + ".txt","w") as f:
                for i in a:
                    f.write(i+"\n")
    ...
    torch.save(retinanet.state_dict(), parser.name + '.pt')
