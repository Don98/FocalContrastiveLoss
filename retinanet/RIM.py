...
class GroupLSTMCell(nn.Module):
	...
	"""
	def __init__(self, inp_size, hidden_size, num_lstms,device):
		super().__init__()
		...
		self.i2h = nn.Conv2d(inp_size, 4 * hidden_size, kernel_size=1, stride=1, padding=0).to(self.device)
		self.h2h = nn.Conv2d(hidden_size, 4 * hidden_size, kernel_size=1, stride=1, padding=0).to(self.device)
	...
	def forward(self, x, hid_state):
		...
		"""
		h, c = hid_state
		# print("Cell shape",x.shape,h.shape)
		x_shape = x.shape
		h_shape = h.shape
		# print("self size",self.inp_size,self.hidden_size)
		# print("self i2h",self.i2h(x.contiguous().view(2,)).shape,self.h2h(h).shape)
		preact = self.i2h(x.contiguous().view(x_shape[0],x_shape[2],1,x_shape[1])).view(x_shape[0],x_shape[1],4*self.hidden_size) + self.h2h(h.contiguous().view(h_shape[0],h_shape[2],1,h_shape[1])).view(h_shape[0],h_shape[1],4*self.hidden_size)
...
class RIMCell(nn.Module):
	def __init__(self, 
		...
		self.key = nn.Conv2d(16, 16, kernel_size=1, stride=1, padding=0).to(self.device)
		# self.value = nn.Linear(input_size, num_input_heads * input_value_size).to(self.device)
		self.value = nn.Conv2d(16, 16, kernel_size=1, stride=1, padding=0).to(self.device)
		...
		if self.rnn_cell == 'GRU':
			...
			self.rnn = GroupLSTMCell(input_value_size, hidden_size, num_units,self.device)
			# self.query = GroupLinearLayer(hidden_size,  input_key_size * num_input_heads, self.num_units)
			self.query = nn.Conv2d(32,  80, kernel_size=1, stride=1, padding=0).to(self.device)
		...
		self.query_ = nn.Conv2d(64,  4, kernel_size=1, stride=1, padding=0).to(self.device)
		self.key_   = nn.Conv2d(64,  4, kernel_size=1, stride=1, padding=0).to(self.device)
		self.value_ = nn.Conv2d(512, 512, kernel_size=1, stride=1, padding=0).to(self.device)
		# self.comm_attention_output = GroupLinearLayer(num_comm_heads * comm_value_size, comm_value_size, self.num_units)
		self.comm_attention_output = nn.Conv2d(512, 512, kernel_size=1, stride=1, padding=0).to(self.device)
	...
	def transpose_for_scores(self, x, num_attention_heads, attention_head_size):
	    ...
	    # return x.view(x.size(0),attention_head_size*x.size(2),x.size(3))
	...
	def input_attention_mask(self, x, h):
		...
		"""
	    # print("input attention shape ",x.shape)
	    # print("size is ",self.num_input_heads * self.input_query_size)
	    batch_size = x.shape[0]
	    # key_layer = self.key(x)
	    key_layer = self.key(x.view(batch_size,16,2,80))
	    key_layer = key_layer.view(key_layer.size(0),key_layer.size(2),key_layer.size(1)*key_layer.size(3))
	    # print("key_layer shape ",key_layer.shape)
	    # value_layer = self.value(x)
	    value_layer = self.key(x.view(batch_size,16,2,80))
	    value_layer = value_layer.view(value_layer.size(0),value_layer.size(2),value_layer.size(1)*value_layer.size(3))
	    # print("value_layer shape ",value_layer.shape)
	    # print("h shape ",h.size(),self.hidden_size,self.input_query_size)
	    # query_layer = self.query(h)
	    query_layer = self.query(h.contiguous().view(batch_size,32,256,16))
	    query_layer = query_layer.view(query_layer.size(0),query_layer.size(2),query_layer.size(1)*query_layer.size(3))
	    # print("query_layer shape ",query_layer.shape)

	    # print("num_input_heads  ",self.num_input_heads, self.input_key_size)
	    # key_layer = self.transpose_for_scores(key_layer,  self.num_input_heads, self.input_key_size)
	    # print("key_layer shape ",key_layer.shape)
	    # print("the value shape ",self.transpose_for_scores(value_layer,  self.num_input_heads, self.input_value_size).shape)
	    ...
	    # print("query_layer shape ",query_layer.shape)
	    # print("key_layer transpose ",key_layer.transpose(-1, -2).shape)
	...
	def communication_attention(self, h, mask):
	    ...
	    # print("the h shape",h.shape)
	    query_layer = self.query_(h.contiguous().view(batch_size,64,256,8))
	    query_layer = query_layer.view(query_layer.size(0),query_layer.size(2),query_layer.size(1)*query_layer.size(3))
	    # query_layer = self.query_(h)
	    # print("the query_layer shape",query_layer.shape)
        
	    # key_layer = self.key_(h)
	    key_layer = self.key_(h.contiguous().view(batch_size,64,256,8))
	    key_layer = key_layer.view(key_layer.size(0),key_layer.size(2),key_layer.size(1)*key_layer.size(3))
	    # print("the key_layer shape",key_layer.shape)
        
	    # value_layer = self.value_(h)
	    value_layer = self.value_(h.contiguous().view(batch_size,512,256,1))
	    value_layer = value_layer.view(value_layer.size(0),value_layer.size(2),value_layer.size(1)*value_layer.size(3))
	    # print("the value_layer shape",value_layer.shape)
	    ...
	    # print("each shape",self.num_comm_heads , self.comm_value_size, self.comm_value_size, self.num_units)
	    # context_layer = self.comm_attention_output(context_layer)
	    context_layer = self.comm_attention_output(context_layer.contiguous().view(batch_size,512,256,1))
	    context_layer = context_layer.view(context_layer.size(0),context_layer.size(2),context_layer.size(1)*context_layer.size(3))
	    # print("context_layer shape",context_layer.shape)
	...
	def forward(self, x, hs, cs = None):
		...
		size = x.size()
		null_input = torch.zeros(size[0], size[1], size[2]).float().to(self.device)
		x = torch.cat((x, null_input), dim = 1)
		# print("null shape ",null_input.shape)
		...
		if cs is not None:
			# print("the inputs ,",inputs.shape, hs.shape, cs.shape)
